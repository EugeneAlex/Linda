...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Alfa
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Alfa
    // Mac: ~/Library/Application Support/Alfa
    // Unix: ~/.Alfa
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Alfa";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "Alfa.conf"));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) //Alfa.conf doesn't exist
    {
        createConf();
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "Alfad.pid"));
