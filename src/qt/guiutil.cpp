...
{
    // Convert Alfa:// to Alfa:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("Alfa://"))
    {
        uri.replace(0, 12, "Alfa:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Alfa.lnk";
}
...
{
    return GetAutostartDir() / "Alfa.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Alfa-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  Alfa-qt [" + tr("command-line options") + "]                     " + "\n";
